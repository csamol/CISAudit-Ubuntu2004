...
@test "1.7.1.4 Ensure all AppArmor Profiles are enforcing (Scored)" {
    ...
    run bash -c "apparmor_status | grep profiles | grep '0 processes are in complain mode .'"
    #[ "$status" -eq 0 ]
    # As we need ssm agent it has to be set to 1 as the ssm tools will be in comaplin mode
    [ "$status" -eq 1 ]
    run bash -c "apparmor_status | grep processes | grep '0 processes are unconfined'"
    [ "$status" -eq 0 ]
}
